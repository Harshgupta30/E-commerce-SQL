...
//     rhome.open("post",`./sellerrev?id=${s}&un=${req.session.username}`);
...
function gethome(s){
    let home = new XMLHttpRequest();
    home.open("post",`./sellerorder?id=${s}`);
    home.addEventListener("load",function (response){
        console.log(home.responseText);
        fun(home.responseText)
    })
...
function revhome(s){
    let rhome = new XMLHttpRequest();
    // console.log(typeof(s),s);
    rhome.open("post",`./sellerrevord?id=${s}`);
    rhome.addEventListener("load",function (response){
        // console.log(rhome.responseText);
        fun(rhome.responseText)
    })
...
function create(el) {
    ...
    let un = document.createElement("h3");
    let uq = document.createElement("h3");
    let ue = document.createElement("h3");

    un.innerText = `${el.username}`;
    uq.innerText = `Quantity:${el.qn}`;
    ue.innerText = `Email:${el.email}`;
    orderdetail.appendChild(un);
    orderdetail.appendChild(uq);
    orderdetail.appendChild(ue);

    bcard.appendChild(orderdetail)
